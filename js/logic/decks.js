import {DECK_DEFINITIONS, DECKS} from '../definitions/cards.js';
...
import {createAbilityCardBack, createAbilityCardFront, defineModifierCard, UICard} from './cards.js';
...
function resetModifierDeck() {
    window.modifierDeck.removeBlessAndCurse();
...
export function loadModifierDeck() {
    ...
    deck.removeBlessAndCurse = function () {
        for (let i = 0; i < deck.discard.length; i++) {
            if (this.discard[i].cardType === CARD_TYPES_MODIFIER.BLESS ||
                this.discard[i].cardType === CARD_TYPES_MODIFIER.CURSE) {
                //Delete this curse/bless that has been used
        ...
        for (let i = 0; i < deck.drawPile.length; i++) {
            if (this.drawPile[i].cardType === CARD_TYPES_MODIFIER.BLESS ||
                this.drawPile[i].cardType === CARD_TYPES_MODIFIER.CURSE) {
                //Delete this curse/bless that has been used
        ...
        document.body.dispatchEvent(new CustomEvent(
            ...
            {
                detail: {
                    cardType: CARD_TYPES_MODIFIER.BLESS,
                    count: 0
                }
            }));
        ...
        document.body.dispatchEvent(new CustomEvent(
            ...
            {
                detail: {
                    cardType: CARD_TYPES_MODIFIER.CURSE,
                    count: 0
                }
            }));
    ...
    if (loadedDeck) {
        Object.keys(loadedDeck.discard).forEach(c => deck.discard.push(defineModifierCard(MODIFIER_CARDS[loadedDeck.discard[c].cardType.toUpperCase()])));
        Object.keys(loadedDeck.drawPile).forEach(c => deck.drawPile.push(defineModifierCard(MODIFIER_CARDS[loadedDeck.drawPile[c].cardType.toUpperCase()])));
    } else {
...
export function applyDeckSelection(decks, preserveExistingDeckState) {
    ...
    decksToAdd.forEach(function (deck) {
        ...
        deckSpace.oncontextmenu = function (e) {
            e.preventDefault();
            e.stopPropagation();
            resetDeck(deck);
        };
...
function resetDeck(deck) {
    reshuffle(deck, true);
    updateDeckSwitchInitiative(deck.deckId, "??");
    reorderSwitches();
    writeToStorage(deck.name, JSON.stringify(deck));
}
